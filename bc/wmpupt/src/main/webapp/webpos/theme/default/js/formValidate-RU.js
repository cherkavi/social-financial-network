		...
		function validateElement(elem, type)
		{
			requaredMsg='Вы не заполнили все обязательные поля!';
			typeMsg='Некоректно заполненное поле!';
			var flagReturn=true;
			...
			if (tmp.type== null || tmp.type=='undefined') {
				...
				if (!chbChecked) {
					undefTmp.className='focus'; alert(requaredMsg); undefTmp.focus(); flagReturn=false;
				}
			//}
			//alert('tmp.type='+tmp.type);
			} else {
				...
				if (tmp.value) {
					if (!validateType(type,tmp.value)) {

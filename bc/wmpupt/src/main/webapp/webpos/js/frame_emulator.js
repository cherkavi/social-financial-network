 ...
 var headScriptAdd=0;
   ...
   */
   page_request.onerror=function(e){
	   showError(e);
   }
   showWaiter();
   
   var endChar = url.substr(url.length-4);
   if (".JSP" == endChar.toUpperCase()) {
  ...
  function showError(e){
	  console.log(e);
	  alert("Internal server error:"+e.error);
	  hideWaiter();
  }
  ...
  function loadpage(page_request, containerid) {
	if (page_request.readyState == 4){
		if (page_request.status == 200 || window.location.href.indexOf("http") == -1) {
			...
			console.error("request error:"+page_request);
			hideWaiter();
		}
	}
}

...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("ADD", "../crm/club/clubupdate.jsp?type=club&action=add&process=no", "", "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/club/club.jsp?print=Y", "", "") %>

			<!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/club/club.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/club/club.jsp?page=1&") %>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getClubsHeadHTML(find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
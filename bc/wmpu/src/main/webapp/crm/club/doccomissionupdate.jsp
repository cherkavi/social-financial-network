...
<%@page import="bc.objects.bcDocumentObject"%><html>
<head>
	<%= Bean.getMetaContent() %>
...
if (p_process==null || ("".equalsIgnoreCase(p_process))) p_process="empty";
 ...
 if (p_type.equalsIgnoreCase("comission")) {
	  if (p_process.equalsIgnoreCase("no")) {
        ...
        <form action="../crm/club/doccomissionupdate.jsp" name="updateForm" id="updateForm" accept-charset="UTF-8" method="POST" onSubmit="return myValidateForm()">
		        <input type="hidden" name="type" value="comission">
		        <input type="hidden" name="action" value="add2">
		        <input type="hidden" name="process" value="yes">
	        	<input type="hidden" name="id" value="<%=p_id %>">
	        	<input type="hidden" name="id_doc" value="<%=p_id %>">
		<table <%=Bean.getTableDetailParam() %>>
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("name_comission_type", true) %></td><td><%=Bean.getComissionTypeRadio("cd_comission_type", "") %></td>
			</tr>
			<tr>
				<td colspan="6" align="center">
					<%=Bean.getSubmitButtonAjax("../crm/club/doccomissionupdate.jsp") %>
					<%=Bean.getResetButton() %>
					<%=Bean.getGoBackButton("../crm/club/documentspecs.jsp?id=" + p_id) %>
				</td>
			</tr>
		</table>
		</form>
...
<body>
		...
		<table <%=Bean.getTableDetailParam() %>>
			...
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("full_doc", true) %></td><td><input type="text" id="full_doc" name="full_doc" size="90" value="<%=Bean.getDocName(p_id) %>" readonly="readonly" class="inputfield-ro"></td>
		...
		<table <%=Bean.getTableDetailParam() %>>
			...
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("full_doc", false) %></td><td><input type="text" id="full_doc" name="full_doc" size="90" value="<%=Bean.getDocName(p_id) %>" readonly="readonly" class="inputfield-ro"></td>
	  ...
	  // Применить  
		...
		if (p_action.equalsIgnoreCase("add")) {
			...
			%>
			<%= Bean.getCallResultParam("INSERT", callSQL, pParam, "../crm/club/documentspecs.jsp?id=" + p_id + "&id_comission=", "") %>
			<% 	

		} else if (p_action.equalsIgnoreCase("addall")) {

			String callSQL = "{? = call " + Bean.getGeneralDBScheme() + ".PACK_UI_COMISSION.add_all_doc_comission(?,?)}";
			...
			%>
			<%= Bean.getCallResultParam("UPDATE", callSQL, pParam, "../crm/club/documentspecs.jsp?id=" + p_id, "") %>
			<% 	
			...
			%>
			<%= Bean.getCallResultParam("DELETE", callSQL, pParam, "../crm/club/documentspecs.jsp?id=" + p_id, "") %>
			<% 	

		} else if (p_action.equalsIgnoreCase("removeall")) {
			
			String callSQL = "{? = call " + Bean.getGeneralDBScheme() + ".PACK_UI_COMISSION.delete_all_doc_comission(?,?)}";
			...
			String[] pParam = new String [3];
	   	    ...
	   	    <%=Bean.showCallResult(
	   	    		...
	   	    		fullResult, 
	   	    		fullResultMessage, 
	   	    		"../crm/club/documentspecs.jsp?id=" + p_id, 
	   	    		"../crm/club/documentspecs.jsp?id=" + p_id, 

...
String prefix = Bean.getDecodeParamPrepare(parameters.get("prefix"));
%>
<head>
	<%=Bean.getMetaContent() %>
	...
	<script>		
		function changeOpener(cdCard, serialNumber, idIssuer, idPaymentSystem){
			window.opener.document.getElementById('<%=prefix%>cd_card1').value = cdCard;
			window.opener.document.getElementById('<%=prefix%>card_serial_number').value = serialNumber;
			window.opener.document.getElementById('<%=prefix%>id_issuer').value = idIssuer;
			window.opener.document.getElementById('<%=prefix%>id_payment_system').value = idPaymentSystem;
			window.opener.document.getElementById('<%=prefix%>cd_card1').className = "inputfield_modified";
			window.opener.document.getElementById('<%=prefix%>card_serial_number').className = "inputfield_modified";
			window.close();
		}

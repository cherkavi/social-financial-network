...
String tagPrivilegeType = "_PRIVILEGE_TYPE";
...
String id_priv_type	= Bean.getDecodeParam(parameters.get("id_priv_type"));
if (id_priv_type==null) {
	id_priv_type = "";
}
id_priv_type		= Bean.checkFindString(pageFormName + tagPrivilegeType, id_priv_type, l_role_page);
...
/* вибираємо тип дії (добавити, видалити...)*/
   ...
   <form action="../crm/security/rolesmenuupdate.jsp" name="updateForm" id="updateForm" accept-charset="UTF-8" method="POST">
    ...
    <tr>
		<td><%= Bean.roleXML.getfieldTransl("name_menu_element", false) %></td> <td><select name="id_menu" class="inputfield"><%= Bean.getRoleMenuOptions(role.getValue("CD_MODULE_TYPE"), id_role) %></select></td>
	</tr>
    <tr>
		<td><%= Bean.roleXML.getfieldTransl("name_privilege_type", false) %></td> <td><select name="id_privilege_type" class="inputfield"><%= Bean.getPrivilegeTypeOptions("", false) %></select></td>
	</tr>
	<tr>
	    ...
	    <tr>
			<td><%= Bean.roleXML.getfieldTransl("name_privilege_type", false) %></td> <td><select name="id_privilege_type" class="inputfield"><%= Bean.getPrivilegeTypeOptions(privilege.getValue("ID_PRIVILEGE_TYPE"), false) %></select></td>
		</tr>
		<tr>
			<td colspan="6" align="center">
				<%=Bean.getSubmitButtonAjax("../crm/security/rolesmenuupdate.jsp") %>
		...
		<tr>
			<td>&nbsp;</td>
			<td>
				<%=Bean.getSelectOnChangeBeginHTML("id_priv_type", "../crm/security/rolesmenuupdate.jsp?id_role=" + id_role + "&process=no&action=editall&role_page=1&", Bean.roleXML.getfieldTransl("NAME_PRIVILEGE_TYPE", false)) %>
					<%=Bean.getSelectOptionHTML(id_priv_type, "", "") %>
					<%=Bean.getSelectOptionHTML(id_priv_type, "0", Bean.roleXML.getfieldTransl("l_access_denied", false)) %>
					<%= Bean.getPrivilegeTypeOptions(id_priv_type, false) %>
				<%=Bean.getSelectOnChangeEndHTML() %>
			</td>
			<!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName + tagRolesEdit, "../crm/security/rolesmenuupdate.jsp?id_role=" + id_role + "&process=no&action=editall&id_priv_type=" + id_priv_type + "&", "role_page") %>
			</tr>
		</table>
		<%=role.getRoleMenuListFullHTML("", id_priv_type, l_role_page_beg, l_role_page_end) %>
	<%
    	} 
    else {

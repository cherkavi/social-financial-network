...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("PRINT", "../crm/finance/tax.jsp?print=Y", "", "") %>

		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/finance/tax.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/finance/tax.jsp?page=1&") %>
		</tr>
	</table>
<% } else { %>
	<table <%= Bean.getTableMenuParam() %>>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getTaxHeadHTML(find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
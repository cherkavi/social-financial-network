...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
			<%= Bean.getPageHeader() %>
		    <%= Bean.getMenuButton("PRINT", "../crm/reports/rep_selected.jsp?print=Y", "", "") %>
	
		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/reports/rep_selected.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/reports/rep_selected.jsp?page=1&") %>
	
			<%=Bean.getSelectOnChangeBeginHTML("module_type", "../crm/reports/rep_selected.jsp?page=1&", Bean.reportXML.getfieldTransl("cd_module_type", false)) %>
			 	<%= Bean.getSysModuleTypeOptions(module_type, true) %>
			<%=Bean.getSelectOnChangeEndHTML() %>
	
			<%=Bean.getSelectOnChangeBeginHTML("report_kind", "../crm/reports/rep_selected.jsp?page=1&", Bean.reportXML.getfieldTransl("cd_report_kind", false)) %>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	...
	<%= Bean.header.getReportsSelectedHTML(find_string, module_type, report_kind, l_page_beg, l_page_end, print) %>
<%} %>
</body>

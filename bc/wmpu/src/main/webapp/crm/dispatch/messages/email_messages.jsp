...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("ADD", "../crm/dispatch/messages/email_messagesupdate.jsp?type=general&action=add&process=no", "", "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/dispatch/messages/email_messages.jsp?print=Y", "", "") %>
	
		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/dispatch/messages/email_messages.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/dispatch/messages/email_messages.jsp?page=1&") %>
	
			<%=Bean.getSelectOnChangeBeginHTML("message_oper_type", "../crm/dispatch/messages/email_messages.jsp?page=1", Bean.messageXML.getfieldTransl("name_ds_message_oper_type", false)) %>
				<%= Bean.getDispatchMessageOperationTypeOptions(message_oper_type, true) %>
			<%=Bean.getSelectOnChangeEndHTML() %>

			<%=Bean.getSelectOnChangeBeginHTML("dispatch_kind", "../crm/dispatch/messages/email_messages.jsp?page=1", Bean.messageXML.getfieldTransl("name_ds_sender_dispatch_kind", false)) %>
				<%= Bean.getDispatchKindOptions(dispatch_kind, true) %>
			<%=Bean.getSelectOnChangeEndHTML() %>

			<%=Bean.getSelectOnChangeBeginHTML("cd_state", "../crm/dispatch/messages/email_messages.jsp?page=1", Bean.smsXML.getfieldTransl("name_sms_state", false)) %>
				<%= Bean.getMeaningFromLookupNameOrderByNymberValueOptions("SEND_MESSAGE_STATE", cd_state, true) %>
			<%=Bean.getSelectOnChangeEndHTML() %>
	
			<%=Bean.getSelectOnChangeBeginHTML("is_archive", "../crm/dispatch/messages/email_messages.jsp?page=1", Bean.smsXML.getfieldTransl("is_archive", false)) %>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) {
	if (Bean.hasEditMenuPermission(pageFormName)) {
		...
		<% } %>
	<%= Bean.header.getDispatchEmailMessagesHeaderHTML(operation_type, find_string, dispatch_kind, message_oper_type, cd_state, is_archive, l_beg, l_end, print) %>
<%  
} %>
</div></div>
</body>

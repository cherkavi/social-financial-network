...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("ADD", "../crm/dispatch/settings/email_profileupdate.jsp?type=general&action=add&process=no", "", "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/dispatch/settings/email_profiles.jsp?print=Y", "", "") %>

		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/dispatch/settings/email_profiles.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/dispatch/settings/email_profiles.jsp?page=1&") %>
...
<% if (Bean.hasEditMenuPermission(pageFormName)) { %>
	<% Bean.header.setDeleteHyperLink("../crm/dispatch/settings/email_profileupdate.jsp?type=general&action=remove&process=yes&id=",Bean.emailXML.getfieldTransl("h_delete_profile", false),"ID_EMAIL_PROFILE","NAME_EMAIL_PROFILE"); %>
<%} else { %>
	<% Bean.header.setDeleteHyperLink("","",""); %>
<%} %>

<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getEmailProfileHTML(cd_state, find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
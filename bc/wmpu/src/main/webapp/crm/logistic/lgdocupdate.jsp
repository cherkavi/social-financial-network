...
<body>
<%
Logger LOGGER = Logger.getLogger( "../crm/logistic/lgdocupdate.jsp" );
...
if ("BON_CARD".equalsIgnoreCase(lg_type)) {
	pageFormName = "vc_lg_card_priv_all";
	specFileName = "cardspecs.jsp";
} else if ("QUESTIONNAIRE".equalsIgnoreCase(lg_type)) {
	pageFormName = "vc_lg_questionnaire_priv_all";
	specFileName = "questspecs.jsp";
} else if ("TERMINAL".equalsIgnoreCase(lg_type)) {
	pageFormName = "vc_lg_terminal_priv_all";
	specFileName = "terminalspecs.jsp";
} else if ("SAM".equalsIgnoreCase(lg_type)) {
	pageFormName = "vc_lg_sam_priv_all";
	specFileName = "samspecs.jsp";
} else if ("OTHER".equalsIgnoreCase(lg_type)) {
	pageFormName = "vc_lg_other_priv_all";
	specFileName = "otherspecs.jsp";
}
...
if (type.equalsIgnoreCase("doc")) {
	...
	if (process.equalsIgnoreCase("no")) {
		...
		<%= Bean.getOperationTitle(
				...
				"Y") 
		%>
    <form action="../crm/logistic/lgdocupdate.jsp" name="updateForm" id="updateForm" enctype="multipart/form-data" accept-charset="UTF-8" method="POST" onSubmit="return validateForm(formData)">
	...
	<table <%=Bean.getTableDetailParam() %>>
		...
		<tr>
			<td colspan="6" align="center">
				<%=Bean.getSubmitButtonMultiPart("../crm/logistic/lgdocupdate.jsp", "submit", "updateForm") %>
				<%=Bean.getResetButton() %>
				<%=Bean.getGoBackButton("../crm/logistic/partners/"+ specFileName + "?id=" + id) %>
			</td>
		</tr>
	</table>
	</form>
    	...
    	<tr>
			...
			<td>
				<% if (!(doc.getValue("FILE_DOC")==null || "".equalsIgnoreCase(doc.getValue("FILE_DOC")))) { %>
					...
					<a href="#" onclick="if (window.confirm('<%=Bean.documentXML.getfieldTransl("l_remove_src_doc", false) %>?')) {ajaxpage('../crm/logistic/lgdocupdate.jsp?lg_type=<%=lg_type %>&type=doc&id=<%=id%>&id_doc=<%=id_doc%>&process=yes&action=remove_src', 'div_main')}" title="<%= Bean.buttonXML.getfieldTransl("button_delete", false) %>">
		...
		if (action.equalsIgnoreCase("add")) {
			...
			if(ServletFileUpload.isMultipartContent(request)){
				...
				for(FileItem file:fileList){
				 ...
				 LOGGER.error(type+" "+process+" "+action+ " ERROR! File not found");
			 }
			...
			if (transfer_ok) {
				...
				pParam[16] = Bean.getDateFormat();
		 		...
		 		%>
				<%= Bean.getCallResultParam("INSERT", callSQL, pParam, "../crm/logistic/partners/"+ specFileName + "?id=" + id + "&id_doc=", "") %>
				<% 	
			} else {%>
				<%=Bean.showCallResult("", "1", errorText, "../crm/logistic/partners/"+ specFileName + "?id=" + id, "", Bean.documentXML.getfieldTransl("h_upload_file_error", false)) %>
			<%}
			...
			%>
			<%= Bean.getCallResultParam("DELETE", callSQL, pParam, "../crm/logistic/partners/"+ specFileName + "?id=" + id, "") %>
			<% 	
			...
			%>
			<%= Bean.getCallResultParam("DELETE", callSQL, pParam, "../crm/logistic/lgdocupdate.jsp?type=doc&process=no&action=edit&id=" + id + "&id_doc=" + id_doc + "&lg_type" + lg_type, "") %>
			<% 	
			...
			if (transfer_ok) {
				...
				%>
				<%= Bean.getCallResultParam("UPDATE", callSQL, pParam, "../crm/logistic/partners/"+ specFileName + "?id=" + id, "") %>
				<%
			} else {%>
				<%=Bean.showCallResult("", "1", errorText, "../crm/logistic/partners/"+ specFileName + "?id=" + id, "", Bean.documentXML.getfieldTransl("h_upload_file_error", false)) %>
			<%}

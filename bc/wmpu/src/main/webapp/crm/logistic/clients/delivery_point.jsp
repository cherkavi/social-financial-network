...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("ADD", "../crm/logistic/clients/delivery_pointupdate.jsp?type=general&action=add&process=no", "", "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/logistic/clients/delivery_point.jsp?print=Y", "", "") %>
			
		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/logistic/clients/delivery_point.jsp?", "page") %>
		</tr>
	</table>
	
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/logistic/clients/delivery_point.jsp?page=1&") %>
		</tr>
	</table>
<% } else { %>
	<table <%= Bean.getTableMenuParam() %>>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getLogisticDeliveryPointsHTML(find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
			...
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/logistic/partners/quest.jsp?print=Y", "", "") %>

		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/logistic/partners/quest.jsp?", "page") %>
		</tr>
	</table>
	
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/logistic/partners/quest.jsp?page=1&") %>
		</tr>
	</table>
<% } else { %>
	<table <%= Bean.getTableMenuParam() %>>
...
<% if (Bean.hasEditMenuPermission(pageFormName)) { %>
	<% Bean.header.setDeleteHyperLink("../crm/logistic/partners/questupdate.jsp?type=general&action=remove&process=yes&id=",Bean.logisticXML.getfieldTransl("h_action_questionnaires_add", false),"ID_LG_RECORD"); %>
<%} else { %>
	<% Bean.header.setDeleteHyperLink("","",""); %>
<%} %>
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getLogisticClubCardsHeadHTML("QUESTIONNAIRE", find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("RUN", "../crm/logistic/partners/analyticupdate.jsp?type=general&action=runall&process=yes", Bean.logisticXML.getfieldTransl("h_update_all_analytics", false), "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/logistic/partners/analytics.jsp?print=Y", "", "") %>

		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/logistic/partners/analytics.jsp?", "page") %>
		</tr>
	</table>
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/logistic/partners/analytics.jsp?page=1&") %>
		</tr>
	</table>
<% } else { %>
	<table <%= Bean.getTableMenuParam() %>>
...
<% if (Bean.hasAccessMenuPermission(pageFormName)) { %>
	<%= Bean.header.getLogisticAnalyticsHeaderHTML(find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
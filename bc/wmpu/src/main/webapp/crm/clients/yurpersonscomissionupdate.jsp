...
if (p_code==null || ("".equalsIgnoreCase(p_code))) p_code="empty";
	  ...
	  if (p_process.equalsIgnoreCase("no")) {
		...
		<script>
			var formData = new Array (
					new Array ('id_club_rel', 'varchar2', 1)
				);
		</script>
		...
		<%= Bean.getOperationTitleShort(
				...
				"N") 
		%>
		<form action="../crm/clients/yurpersonscomissionupdate.jsp" name="updateForm" id="updateForm" accept-charset="UTF-8" method="POST">
    		<table <%=Bean.getTableDetailParam() %>>
	    		<tr>
	    			<td><%=Bean.relationshipXML.getfieldTransl("general", true)%></td><td><select name="id_club_rel" id="id_club_rel" class="inputfield"><%= Bean.getClubRelationshipOptions(p_id, "", false) %></select></td>
	    		</tr>
	     		<tr>
	    			<td colspan="2" align="center">
						<%=Bean.getSubmitButton("../crm/clients/yurpersonscomissionupdate.jsp?type=comission&action=add2&process=no&id=" + p_id + "&id_club_rel='+getElementById('id_club_rel').value") %>
						<%=Bean.getGoBackButton("../crm/clients/yurpersonspecs.jsp?id=" + p_id) %>
	    			</td>
	    		</tr>
	    	</table>
		</form>
    	        ...
    	        <%
    	        /*  --- Видалити запис --- */
        } else if (p_action.equalsIgnoreCase("add2")) {
			  
        	String idClubRel = Bean.getDecodeParam(parameters.get("id_club_rel"));
...
<body>
		<script type="text/javascript">
		  ...
		  if ("DEALER-TECHNICAL_ACQUIRER".equalsIgnoreCase(rel_type) ||
			  ...
			  idDealer = rel.getValue("ID_PARTY1");
		  } else if ("OPERATOR-DEALER".equalsIgnoreCase(rel_type)) {
			  idDealer = rel.getValue("ID_PARTY2");
		  }
		  %>
		  
		
		  <%=Bean.getClubRelationshipComissionTypeArray(rel.getValue("CD_CLUB_REL_TYPE"), rel.getValue("CD_JUR_PRS_PRIMARY_TYPE_PARTY1"), rel.getValue("CD_JUR_PRS_PRIMARY_TYPE_PARTY2"), "element_payer", "element_receiver")%>
        ...
        <form action="../crm/clients/yurpersonscomissionupdate.jsp" name="updateForm" id="updateForm" accept-charset="UTF-8" method="POST" onSubmit="return myValidateForm()">
		        <input type="hidden" name="type" value="comission">
		        <input type="hidden" name="action" value="add">
		        <input type="hidden" name="process" value="yes">
	        	<input type="hidden" name="id" value="<%=p_id %>">
	        	<input type="hidden" name="id_club_rel" value="<%=idClubRel %>">
		<table <%=Bean.getTableDetailParam() %>>
			<tr>
			...
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("name_comission_type", true) %></td><td><select name="id_comission_type" id="id_comission_type" class="inputfield" onchange="set_payer_receiver(this);"><%= Bean.getClubRelationishipAllComissionOptions(idClubRel, rel.getValue("CD_JUR_PRS_PRIMARY_TYPE_PARTY1"), rel.getValue("CD_JUR_PRS_PRIMARY_TYPE_PARTY2"), false) %></select></td>
			...
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("full_doc", false) %></td><td><select name="id_doc" class="inputfield"><%= Bean.getDocForClubRelationshipOptions(idClubRel, "", true) %></select></td>
			...
			<tr>
				<td colspan="6" align="center">
					<%=Bean.getSubmitButtonAjax("../crm/clients/yurpersonscomissionupdate.jsp") %>
					<%=Bean.getResetButton() %>
					<%=Bean.getGoBackButton("../crm/clients/yurpersonscomissionupdate.jsp?type=comission&action=add&process=no&id=" + p_id) %>
				</td>
			</tr>
		</table>
		</form>
		...
		<table <%=Bean.getTableDetailParam() %>>
			...
			<tr>
				...
				<% if (!(comission.getValue("ID_JUR_PRS_PAYER").equalsIgnoreCase(p_id))) { %>
			...
			<tr>
				<td><%= Bean.comissionXML.getfieldTransl("jur_prs_receiver", false) %>
				<% if (!(comission.getValue("ID_JUR_PRS_RECEIVER").equalsIgnoreCase(p_id))) { %>
	  ...
	  // Применить  
		...
		if (p_action.equalsIgnoreCase("add")) {
			...
			%>
			<%= Bean.getCallResultParam("INSERT", callSQL, pParam, "../crm/clients/yurpersonspecs.jsp?id=" + p_id + "&id_comission=", "") %>
			<% 	
			...
			String callSQL = "{? = call " + Bean.getGeneralDBScheme() + ".PACK_UI_COMISSION.add_all_jur_comission(?,?,?)}";
			...
			%>
			<%= Bean.getCallResultParam("UPDATE", callSQL, pParam, "../crm/clients/yurpersonspecs.jsp?id=" + p_id, "") %>
			<% 	
			...
			%>
			<%= Bean.getCallResultParam("DELETE", callSQL, pParam, "../crm/clients/yurpersonspecs.jsp?id=" + p_id, "") %>
			<% 	
			...
			String callSQL = "{? = call " + Bean.getGeneralDBScheme() + ".PACK_UI_COMISSION.delete_all_jur_prs_comission(?,?,?)}";
			...
			String callSQL = "";
	 		...
	 		String[] pParam = new String [3];
	   	    ...
	   	    <%=Bean.showCallResult(
	   	    		...
	   	    		fullResult, 
	   	    		fullResultMessage, 
	   	    		"../crm/clients/yurpersonspecs.jsp?id=" + p_id, 
	   	    		"../crm/clients/yurpersonspecs.jsp?id=" + p_id, 

...
<% if ("N".equalsIgnoreCase(print)) { %>
	<table <%= Bean.getTableMenuParam() %>>
		<tr>
		    ...
		    <%= Bean.getMenuButton("ADD", "../crm/clients/samupdate.jsp?type=general&action=add&process=no", "", "") %>
			<% } %>
		    <%= Bean.getMenuButton("PRINT", "../crm/clients/sam.jsp?print=Y", "", "") %>
	
		    <!-- Вывод страниц -->
			<%= Bean.getPagesHTML(pageFormName, "../crm/clients/sam.jsp?", "page") %>
		</tr>
	</table>
	
	<table <%= Bean.getTableMenuFilter() %>>
		<tr>
			<%= Bean.getFindHTML("find_string", find_string, "../crm/clients/sam.jsp?page=1&") %>
	
			<%=Bean.getSelectOnChangeBeginHTML("cd_type_filtr", "../crm/clients/sam.jsp?page=1", Bean.samXML.getfieldTransl("name_sam_type", false)) %>
				<%= Bean.getSAMTypeOptions(cd_type_filtr, true) %>
...
<%= Bean.header.getSAMHeadHTML(cd_type_filtr, find_string, l_beg, l_end, print) %>
<%} %>
</div></div>
</body>
</html>
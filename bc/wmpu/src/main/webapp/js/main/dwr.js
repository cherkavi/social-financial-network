...
function dwr_make_jur_adr_onload(){
	...
	responseUtility.dwr_make_ser_place_adr(client_transport, get_jur_response_onload);
}

function get_jur_response_onload(transport_from_server){
	load_address_short(transport_from_server,'JUR_ADR_CODE_COUNTRY','JUR_ADR_ID_OBLAST');
	dwr_make_jur_adr(lSessionId);
}
...
function dwr_make_jur_adr(sessionId){
	...
	client_transport.idOblast		= document.getElementById('JUR_ADR_ID_OBLAST').value;
	client_transport.sessionId		= sessionId;
	responseUtility.getAddressArrayShort(client_transport, get_jur_response);
}
...
function dwr_make_fact_adr(sessionId){
	...
	client_transport.sessionId		= sessionId;
	responseUtility.getAddressArrayShort(client_transport, get_fact_response);
	//window.alert('test2');
}

function dwr_make_fact_adr_cur(codeCountry, idOblast, sessionId){
	var client_transport=new Object();
...
function get_fact_response(transport_from_server){
	load_address_short(transport_from_server,'FACT_ADR_CODE_COUNTRY','FACT_ADR_ID_OBLAST');
}
...
function load_address_short(address, selCountry, selOblast){
	var select_codeCountry		= document.getElementById(selCountry);
	var codeCountry				= address.codeCountry;
...
function dwr_make_ser_place_adr(sessionId){
	...
	client_transport.sessionId		= sessionId;
	responseUtility.getAddressArrayShort(client_transport, resp_serv_place_adr);
	//window.alert('test');
}

function dwr_make_ser_place_adr_copy(codeCountry, idOblast, sessionId){
	var client_transport=new Object();
...
function resp_serv_place_adr(transport_from_server){
	load_serv_place_adr(transport_from_server,'ADR_CODE_COUNTRY','ADR_ID_OBLAST');
}

function load_serv_place_adr(address, selCountry, selOblast){
	putToSelect2(selCountry, address.codeCountry, address.countryListCode, address.countryListName);
	putToSelect2(selOblast, address.idOblast, address.oblastListCode, address.oblastListName);
}

...
if (id_menu_element==null || "".equals(id_menu_element)) {
	id_menu_element = "empty";
}
String goBackHyperLink = "";
String pageFormName = "";

if ("710".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_system.jsp";
	pageFormName = "REPORTS_SYSTEM";
} else if ("720".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_accounting.jsp";
	pageFormName = "REPORTS_ACCOUNTING";
} else if ("730".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_marketing.jsp";
	pageFormName = "REPORTS_MARKETING";
} else if ("740".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_po_partners.jsp";
	pageFormName = "REPORTS_PO_PARTNERS";
} else if ("750".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_po_clients.jsp";
	pageFormName = "REPORTS_PO_CLIENTS";
} else if ("790".equalsIgnoreCase(id_menu_element)) {
	goBackHyperLink = "../reports/rep_selected.jsp";
...
jQuery.fn.selectID_DEALER = function(field){
return this.each(function(){
var values_ID_DEALER = [
 { value: 'КП "Колос"', data: '605' },
 { value: 'ООО Агент', data: '607' },
 { value: 'Отд.№2 ГенБанк г.Москва', data: '54' },
 { value: 'ПО "Поток"', data: '0' },
 { value: 'Пчелка', data: '1' },
 { value: 'Тестовый партнер', data: '655' }];
$("#"+field+"_name")
.autocomplete({
source: values_ID_DEALER,
lookupLimit: 10,
minLength:0,
select: function( event , ui ) {
 document.getElementById(field).value =ui.item.data; /*$("#"+field ).val = ui.item.data;*/ /*alert( 'You selected: value=' + ui.item.value + ', data=' +ui.item.data );*/
}
})
.focus(function(){
...
function getID_DEALER(field){
jQuery(document).ready(function(){
jQuery("#"+field+"_name").selectID_DEALER(field);
});
}
getID_DEALER("ID_DEALER");
			</script>
	        <%
	        ...
	        if ("WORKS".equalsIgnoreCase(report_variables.getReportState())) {
		        ...
		        if(report_variables.getVariablesCount()>0){
		        	out.println("<center>");
		        	out.println("<form action=\""+url+"\" target=\"_report\">");

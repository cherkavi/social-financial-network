...
public class CalculatorWithoutHeadString extends ActionStepCalculator{
	...
	/** взять параметр-источник (String) вырезать из него заголовок с указанным кол-вом символов и оставшуюся часть положить в приемник 
	 ...
	 * */
	public CalculatorWithoutHeadString(String parameter_source,String parameter_destination, int count){
	...
	public boolean calculate(Action action) {
		...
		try{
			action.setParameter(field_parameter_destination, ((String)action.getParameter(field_parameter_source)).substring(this.field_count));
			return_value=true;
		}catch(Exception ex){
			return_value=false;
		}

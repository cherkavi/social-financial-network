...
public class CalculatorHeadString extends ActionStepCalculator{
	...
	/** взять параметр-источник (String) получить заголовок с указанным кол-вом символов и положить в приемник 
	 ...
	 * */
	public CalculatorHeadString(String parameter_source,String parameter_destination, int count){
	...
	public boolean calculate(Action action) {
		...
		try{
			action.setParameter(field_parameter_destination, ((String)action.getParameter(field_parameter_source)).substring(0,this.field_count));
			return_value=true;
		}catch(Exception ex){
			return_value=false;
		}

...
public class bcCallCenterSettingObject extends bcObject {
	private final static Logger LOGGER=Logger.getLogger(bcCallCenterSettingObject.class);
	private Map<String,Comparable<String>> fieldHm = new HashMap<String, Comparable<String>>();
	
	private String idSetting;
	
	public bcCallCenterSettingObject(String pIdSetting) {
		this.idSetting = pIdSetting;
		getFeature();
	}
	
	private void getFeature() {
		String featureSelect = " SELECT * FROM " + getGeneralDBScheme() + ".VC_CC_SETTING_ALL WHERE id_cc_setting = ?";
		fieldHm = getFeatures2(featureSelect, this.idSetting, false);
	}
    ...
    public String getWarningTypeCheckBoxes(boolean readOnly) {
    	ArrayList<bcFeautureParam> pParam = initParamArray();
    	pParam.add(new bcFeautureParam("int", this.idSetting));
    	String mySQL = 
    		" SELECT a.lookup_code, a.meaning, DECODE(b.cd_warning_type, NULL, 'N', 'Y') is_selected " +
			"   FROM (SELECT lookup_code, meaning " +
			"           FROM " + getGeneralDBScheme()+".vc_lookup_values " +
			"          WHERE name_lookup_type='SYS_WARNING_TYPE') a" +
			"   LEFT JOIN " +
			"        (SELECT cd_warning_type" +
			"           FROM " + getGeneralDBScheme()+".v_cc_setting_warning_type" +
			"          WHERE id_cc_setting = ?) b " +
			"   ON (a.lookup_code = b.cd_warning_type)" +
			"  ORDER BY a.meaning";
    	return getCheckBoxesFromQuery(
    			"cd_warning_type",
    			mySQL,
    			pParam,
    			readOnly);
    ...
    public String getSysEventTypeCheckBoxes(boolean readOnly) {
    	ArrayList<bcFeautureParam> pParam = initParamArray();
    	pParam.add(new bcFeautureParam("int", this.idSetting));
    	String mySQL = 
    		" SELECT a.cd_event_type, a.desc_event_type, DECODE(b.cd_event_type, NULL, 'N', 'Y') is_selected " +
    		"   FROM (SELECT cd_event_type, desc_event_type " +
    		"           FROM " + getGeneralDBScheme()+".vc_sys_event_type_all) a" +
    		"   LEFT JOIN " +
    		"        (SELECT cd_event_type" +
    		"           FROM " + getGeneralDBScheme()+".v_cc_setting_event_type" +
    		"          WHERE id_cc_setting = ?) b " +
    		"   ON (a.cd_event_type = b.cd_event_type)" +
    		"  ORDER BY a.desc_event_type";
    	return getCheckBoxesFromQuery(
    			"cd_event_type",
    			mySQL,
    			pParam,
	...
	public String getDSMessagesHTML(String pFind, String pMessageType, String pMessageState, String pIsArchive, String p_beg, String p_end) {
		ArrayList<bcFeautureParam> pParam = initParamArray();

...
public class bcLGPromoterPenaltyWriteOffObject extends bcObject {
	private final static Logger LOGGER=Logger.getLogger(bcLGPromoterPenaltyWriteOffObject.class);
	private Map<String,Comparable<String>> fieldHm = new HashMap<String, Comparable<String>>();
	
	private String idLGPromoterPayment;
	
	public bcLGPromoterPenaltyWriteOffObject(String pIdLGPromoterPayment) {
    ...
    public String getPaymentLinesHTML(String pFindString, String pLineState, String p_beg, String p_end) {
        ...
        if (!isEmpty(pFindString)) {
    		mySQL = mySQL + 
   				...
   				"      UPPER(date_begin_sale_frmt) LIKE UPPER('%'||?||'%') OR " +
   				"      UPPER(date_end_sale_frmt) LIKE UPPER('%'||?||'%'))";
    		for (int i=0; i<3; i++) {

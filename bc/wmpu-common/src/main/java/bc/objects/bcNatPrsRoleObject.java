...
public class bcNatPrsRoleObject extends bc.objects.bcObject {
	private final static Logger LOGGER=Logger.getLogger(bcNatPrsRoleObject.class);
    
	private Map<String,Comparable<String>> fieldHm = new HashMap<String, Comparable<String>>();
	
	private String idNatPrsRole;
	
	public bcNatPrsRoleObject(String pIdNatPrsRole) {
		this.idNatPrsRole = pIdNatPrsRole;
		getFeature();
	}
	
	private void getFeature() {
		String featureSelect = " SELECT * FROM " + getGeneralDBScheme() + ".vc_nat_prs_role_all WHERE id_nat_prs_role = ?";
        ...
        try{
            ...
            while (rset.next())
            {
            	...
            	html.append("<spant title=\""+rset.getString("FILE_NAME")+"\">" + lFileName + "</span>&nbsp;" + src_doc);
         	   	...
         	   	html.append(getDeleteFileImageHTML(myDeleteLink, "div_action_big", documentXML.getfieldTransl("l_remove_doc", false) + " \\\'" + rset.getString("FILE_NAME") + "\\\'"));

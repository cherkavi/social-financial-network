...
public class wpFNInvoiceObject extends wpObject {
	private final static Logger LOGGER=Logger.getLogger(wpFNInvoiceObject.class);
	private Map<String,Comparable<String>> fieldHm = new HashMap<String, Comparable<String>>();
	
	private String idInvoice;
	
	public wpFNInvoiceObject(String pIdInvoice) {
    ...
    public String getContentHTML(String pFind, String p_beg, String p_end) {
    	...
    	if (!isEmpty(pFind)) {
    		mySQL = mySQL + 
    			...
    			"      UPPER(total_amount_frmt) LIKE UPPER('%'||?||'%')) ";
    		for (int i=0; i<3; i++) {
        ...
        try{
            ...
            if (hasEditPermission && "ISNT_PAID".equalsIgnoreCase(this.getValue("CD_FN_INVOICE_STATE"))) {
            ...
            while (rset.next()) {
            	...
            	if ("ISNT_PAID".equalsIgnoreCase(this.getValue("CD_FN_INVOICE_STATE"))) {
            		if (hasEditPermission && "Y".equalsIgnoreCase(rset.getString("CAN_CHANGE_DATA"))) {
		            	String myHyperLink = "report/operationupdate.jsp?type=invline&id_invoice="+this.idInvoice+"&id_invoice_line="+rset.getString("ID_INVOICE_LINE");
		            	String myDeleteLink = "report/operationupdate.jsp?type=invline&id_invoice="+this.idInvoice+"&id_invoice_line="+rset.getString("ID_INVOICE_LINE")+"&action=remove&process=yes";
		                html.append(getWebposDeleteButtonHTML(myDeleteLink, clearingXML.getfieldTransl("h_delete_invoice_line", false), rset.getString("NAME_NOMENKL"), "div_invoice_line"));
		            	html.append(getWebposEditButtonHTML(myHyperLink, "div_invoice_line"));
            		} else {
